...
def compute_statistics(A):
    ...
    minimum = min(A)
    average = total / len(A)
    print(total, maximum, minimum, average)

compute_statistics(A)

# print(sum(A))  # Output: 30
# print(max(A))  # Output: 10
# print(min(A))  # Output: 2
# print(sum(A) / len(A))  # Output: 6.0
...
def count_labels(labels):
    """
    Return a dictionary with counts of each label.
    """
...
# plt.bar(df.index, df['a'], color=['blue', 'red'])
